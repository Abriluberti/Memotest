...
@Component({
  ...
  templateUrl: './listado-h.page.html',
  styleUrls: ['./listado-h.page.scss'],
})
export class ListadoHPage implements OnInit {
  ...
  async obtenerMejoresRegistros() {
    const querySnapshot = await getDocs(query(
      collection(this.firestore, 'registrosDeTiempoHerramientas'),
      orderBy('tiempo'),
      limit(5)
    ));
